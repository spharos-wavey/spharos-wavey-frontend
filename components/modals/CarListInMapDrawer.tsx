...
export default function CarListInMapDrawer(props:{data: carInMapType[], isOpen:boolean, setIsOpen:Dispatch<SetStateAction<boolean>>, zoneName:string}) {
  ...
  return (
    <>
    {
      isOpen ? <div className={style.drawerOverlay} onClick={()=>setIsOpen(false)}></div>
      : null
    }
    <div className={isOpen ? `${style.drawerContainer} ${style.open}` : `${style.drawerContainer} ${style.close}`}>
    <CloseOrSlideBtn onClick={()=>setIsOpen(false)}/>
      <div className={style.drawerHeader}>
        <div className={style.drawerTitle}>검색된 차량 {data.length}대</div>
        <div className={style.drawerLocation}><Image src='/assets/images/icons/map.svg' alt='locationicon' width={12} height={12}/>{zoneName}</div>
      </div>
      <div className={style.drawerBody}>
        {data.map((item)=>{
          return (
            <div key={item.vehicleId} className={style.drawerItem} onClick={()=>handleDetail(item.vehicleId)}>
              ...
              <div className={style.drawerItemImg}>
                ...
                <Image src={item.vehicleImage} alt={item.carName} width={100} height={60}/>
                </div>
              </div>
            </div>
          )
